// backend/seed.js (FULL DATA + HASH PASSWORD + OPTIONAL UserProgress)
...
let UserProgress; // optional
try { UserProgress = require('./models/UserProgress'); }
catch { console.log('ℹ️  Không tìm thấy models/UserProgress.js - sẽ bỏ qua seed progress.'); }
...
// -----------------------------
// LESSONS (FULL from your previous data)
// -----------------------------
const LESSON_DATA = [
  // ===== HTML LEVEL 1 =====
  {
...
async function run() {
  ...
  console.log('🔄 Xoá dữ liệu cũ (Language, Lesson, User, UserProgress)...');
  const deletions = [
    Language.deleteMany({}),
    Lesson.deleteMany({}),
    User.deleteMany({}),
  ];
  if (UserProgress) deletions.push(UserProgress.deleteMany({}));
  await Promise.all(deletions);

  console.log('🏗️ Tạo Languages...');
  await Language.insertMany(LANGUAGE_DATA);

  console.log(`🧩 Tạo Lessons (tổng ${LESSON_DATA.length})...`);
  await Lesson.insertMany(LESSON_DATA);

  console.log(`👥 Hash mật khẩu & tạo Users (tổng ${USER_DATA.length})...`);
  const usersHashed = await Promise.all(
    USER_DATA.map(async (u) => ({ ...u, password: await bcrypt.hash(u.password, 10) }))
  );
  const createdUsers = await User.insertMany(usersHashed, { ordered: true });

  const byLang = LESSON_DATA.reduce((acc, cur) => {
    acc[cur.langId] = (acc[cur.langId] || 0) + 1;
    return acc;
  }, {});
  console.log('📊 Số bài theo ngôn ngữ:', byLang);

  if (UserProgress) {
    console.log('⏱️  Tạo UserProgress demo...');
    const now = new Date();
    const dateKey = now.toISOString().slice(0, 10);

    const htmlLessons = LESSON_DATA.filter(l => l.langId === 'html').map(l => l.id);
    const cssLessons = LESSON_DATA.filter(l => l.langId === 'css').map(l => l.id);

    const dev = createdUsers.find(u => u.email === 'devluyen@learncode.com');
    const hvA = createdUsers.find(u => u.email === 'userA@learncode.com');
    ...
    if (dev) {
      docs.push({
        userId: dev._id,
        langId: 'html',
        completedLessonIds: htmlLessons.slice(0, 5),
        timeStats: htmlLessons.slice(0, 6).map((lid, i) => ({ lessonId: lid, totalSeconds: 600 + i * 120 })),
        daily: [{ date: dateKey, seconds: 60 * 45 }],
      ...
      docs.push({
        userId: dev._id,
        langId: 'css',
        completedLessonIds: cssLessons.slice(0, 2),
        timeStats: cssLessons.slice(0, 3).map((lid, i) => ({ lessonId: lid, totalSeconds: 300 + i * 90 })),
        daily: [{ date: dateKey, seconds: 60 * 20 }],
        lastHeartbeatAt: null,
        lastLessonId: null,
      });
    }
    ...
    if (hvA) {
      docs.push({
        userId: hvA._id,
        langId: 'html',
        completedLessonIds: htmlLessons.slice(0, 2),
        timeStats: htmlLessons.slice(0, 3).map((lid, i) => ({ lessonId: lid, totalSeconds: 200 + i * 60 })),
        daily: [{ date: dateKey, seconds: 60 * 15 }],
        lastHeartbeatAt: null,
        lastLessonId: null,
      });
    }
    ...
    if (docs.length) {
      await UserProgress.insertMany(docs);
      console.log(`✅ Đã tạo ${docs.length} bản ghi UserProgress`);
    } else {
      console.log('ℹ️  Không có UserProgress do không tìm thấy model hoặc user phù hợp.');
    }
  }
